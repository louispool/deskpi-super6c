...
*Refer to the [Usage Guide](USAGE.md) after installation of the complete logstack for information on how to use OpenSearch Dashboards to examine logs.*
...
You can check the health of the Opensearch cluster by querying the REST API endpoint, which is exposed via an IngressRoute on the local network 
(in this example assumed to be *opensearch.deskpi.localnet*).
```shell
...
A few useful `curl` commands for interacting with the OpenSearch REST API (assuming it is exposed at `https://opensearch.deskpi.localnet`):

#### 1. List all indices
```bash
curl -u admin "https://opensearch.deskpi.localnet/_cat/indices?v"
```

#### 2. List the contents of an index
```bash
curl -u admin "https://opensearch.deskpi.localnet/<index-name>/_search?pretty" -H 'Content-Type: application/json' -d '{ "size": 10, "sort": [{ "@timestamp": "desc" }] }' | jq
```

#### 3. GET an index
```bash
curl -u admin -X GET "https://opensearch.deskpi.localnet/<index-name>"
```

#### 4. DELETE an index
```bash
curl -u admin -X DELETE "https://opensearch.deskpi.localnet/<index-name>"
```
